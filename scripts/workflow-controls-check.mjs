...
for(const [name,text] of [['sw.js',sw],['sw_47.js',legacySw]]){
  assert(/const CACHE_HOTFIX='(?:workflow1|audit1|reads1|trainerworkflow1)'/.test(text),`${name} não possui cache compatível com workflow-controls ou evolução posterior.`);
  has(text,'security-hardening-v10_10_9.js?v=10.10.10-security8',`${name} não prepara o hardening security8.`);
  has(text,'legacy-student-link-repair-v10_10_10.js?v=10.10.10-legacy-links6',`${name} não prepara o reconciliador legacy-links6.`);
