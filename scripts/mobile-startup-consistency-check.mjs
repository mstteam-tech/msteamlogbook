...
const files=['boot_v10.js','config_v10_7.js','update_v10_10_9.js','sw.js','sw_47.js','modules/v107-invites.js','modules/registration-integrity-v10_10_9.js','modules/trainer-inbox-payments-v10_10_12.js','modules/photo-quality-download-v10_10_9.js','modules/heic-report-conversion-v10_10_12.js'];
...
has(updater,'heic-report-conversion-v10_10_12.js?v=10.10.12-heic1','Atualizador não renova o fallback HEIC.');
for(const [name,text] of [['sw.js',sw],['sw_47.js',bridge]]){
  ...
  has(text,"./modules/heic-report-conversion-v10_10_12.js?v=10.10.12-heic1",`${name} não prepara o fallback HEIC.`);
  has(text,'https://cdn.jsdelivr.net',`${name} CSP não autoriza o conversor HEIC fixado.`);
  has(text,"worker-src 'self' blob:",`${name} CSP bloqueia o worker local do conversor HEIC.`);
...
has(heic,"const VERSION='10.10.12-heic1'",'Fallback HEIC não está na revisão esperada.');
has(heic,"const MAX_HEIC_BYTES=25*1024*1024",'Fallback HEIC perdeu o limite de 25 MB.');
has(heic,"heic2any@0.0.4/dist/heic2any.min.js",'Conversor HEIC não está fixado em versão imutável.');
has(heic,"LIB_INTEGRITY='sha512-",'Conversor HEIC não usa Subresource Integrity.');
has(heic,"script.crossOrigin='anonymous'",'SRI do conversor HEIC não usa CORS anônimo.');
has(heic,"toType:'image/jpeg',quality:.94",'HEIC não é convertido localmente para JPEG.');
