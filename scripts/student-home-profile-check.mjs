...
assert(syntax.status===0,`Módulo da home possui JavaScript inválido: ${String(syntax.stderr||'').trim()}`);
const mod=read(modulePath),config=read('config_v10_7.js'),updater=read('update_v10_10_9.js'),sw=read('sw.js'),sw47=read('sw_47.js'),storage=read('firebase/storage_6.rules'),version=JSON.parse(read('version.json'));
...
assert(/const CACHE_HOTFIX='studenthome1'/.test(sw)&&/const CACHE_HOTFIX='studenthome1'/.test(sw47),'Cache da home não foi rotacionado.');
assert(sw===sw47,'sw.js e sw_47.js divergiram.');

if(fail.length){console.error('FALHA — student home/profile\n- '+fail.join('\n- '));process.exit(1);}
console.log('APROVADO — home do aluno: notificações, avatar/apelido moderáveis, 2 protocolos e selo confidencial validados.');
