...
const core=read('app_v10_10_7_core.js');
const updater=read('update_v10_10_7.js');
const worker=read('sw.js');
const bridge=read('sw_47.js');
const manifest=JSON.parse(read('manifest.json'));
const version=JSON.parse(read('version.json'));

assert(version.version==='10.10.7','version.json incorreto');
...
assert(html.includes('update_v10_10_7.js?v=10.10.7'),'gerenciador de atualização não é carregado');
assert(html.includes('manifest.json?v=10.10.7'),'manifesto estável não é utilizado');
assert(html.includes('modules/v107-core.js?v=10.10.7'),'módulos estáveis não possuem bust de versão');
...
assert(worker.includes("const APP_VERSION='10.10.7'"),'versão do Service Worker incorreta');
