#!/usr/bin/env python3
"""Validação estática — Team Bulls v10.7.2."""
...
REQUIRED=[
 'index.html','config_v10_7.js','app_v10_7_2_core.js','styles_v10_7_2.css','interaction_v10_7_2.js',
 'manifest_24.json','sw_37.js','modules/v107-core.js','modules/v107-invites.js','modules/v107-operations.js',
...
def main():
 ...
 for stale in ['manifest_23.json','styles_v10_7_1.css"','app_v10_7_1_core.js"','interaction_v10_7_1.js"','sw_36.js']:
 ...
 for token in ['id="btn-forgot-password"','id="panel-reset"','id="reset-email"','id="btn-reset-password"']:
  if token not in html:fail('interface de recuperação ausente: '+token)
 manifest=json.loads((ROOT/'manifest_24.json').read_text(encoding='utf-8'))
 if manifest.get('start_url')!='./index.html?v=10.7.2':fail('start_url incorreto')
 if manifest.get('orientation')!='portrait-primary':fail('orientação não está em portrait-primary')
 js=['config_v10_7.js','app_v10_7_2_core.js','interaction_v10_7_2.js','sw_37.js','recovery_v10.js','boot_v10.js','viewport_v10_2_1.js','modules/v107-core.js','modules/v107-invites.js','modules/v107-operations.js']
 for name in js:
 ...
 core=(ROOT/'app_v10_7_2_core.js').read_text(encoding='utf-8')
 for token in ['async function sendPasswordReset()','auth.sendPasswordResetEmail(email)',"auth.languageCode='pt-BR'",'Se existir uma conta vinculada a este e-mail']:
  if token not in core:fail('recuperação de senha ausente: '+token)
 css=(ROOT/'styles_v10_7_2.css').read_text(encoding='utf-8')
 ...
 interaction=(ROOT/'interaction_v10_7_2.js').read_text(encoding='utf-8')
 ...
 sw=(ROOT/'sw_37.js').read_text(encoding='utf-8')
 ...
 for path in sorted(ROOT.rglob('*')):
  if path.is_file() and path.name!='SHA256SUMS-v10.7.2.txt' and '.git' not in path.parts:
   sums.append(hashlib.sha256(path.read_bytes()).hexdigest()+'  '+path.relative_to(ROOT).as_posix())
 (ROOT/'SHA256SUMS-v10.7.2.txt').write_text('\n'.join(sums)+'\n',encoding='utf-8')
