...
function secureResponse(response,{html=false}={}){if(!response)return response;const headers=new Headers(response.headers);headers.set('X-Content-Type-Options','nosniff');headers.set('Referrer-Policy','strict-origin-when-cross-origin');headers.set('Cross-Origin-Resource-Policy','same-origin');headers.set('X-Permitted-Cross-Domain-Policies','none');headers.set('Permissions-Policy','camera=(self), microphone=(), geolocation=(), payment=(), usb=()');if(html){headers.set('Content-Security-Policy',CSP);headers.set('X-Frame-Options','DENY');headers.set('Cross-Origin-Opener-Policy','same-origin');headers.set('Cache-Control','no-cache, no-store, must-revalidate');}return new Response(response.body,{status:response.status,statusText:response.statusText,headers});}
...
async function cacheOne(cache,path,required){try{const response=await fetchFresh(path);if(!response.ok)throw new Error(`HTTP ${response.status}: ${path}`);await cache.put(scopedUrl(path),response.clone());return true;}catch(error){console.warn('[Team Bulls] Não foi possível preparar',path,error);if(required)throw error;return false;}}
async function cachePathsWithLimit(cache,paths,{required=false,limit=4}={}){const queue=Array.from(paths||[]);if(!queue.length)return true;let next=0;const failures=[];const worker=async()=>{while(true){const index=next++;if(index>=queue.length)return;try{await cacheOne(cache,queue[index],required);}catch(error){failures.push(error);}}};const count=Math.min(Math.max(1,Math.trunc(Number(limit)||1)),queue.length);await Promise.all(Array.from({length:count},worker));if(required&&failures.length)throw failures[0];return failures.length===0;}
async function prepareShell(){const cache=await caches.open(SHELL_CACHE);await cachePathsWithLimit(cache,REQUIRED_SHELL,{required:true,limit:SHELL_FETCH_CONCURRENCY});await cachePathsWithLimit(cache,OPTIONAL_SHELL,{required:false,limit:OPTIONAL_FETCH_CONCURRENCY});return true;}
...
self.addEventListener('install',event=>{event.waitUntil((async()=>{await prepareShell();await self.skipWaiting();})());});
self.addEventListener('activate',event=>{event.waitUntil((async()=>{const keys=await caches.keys();await Promise.all(keys.filter(key=>key.startsWith(CACHE_PREFIX)&&![SHELL_CACHE,RUNTIME_CACHE,AUDIO_CACHE_NAME].includes(key)).map(key=>caches.delete(key)));if(self.registration.navigationPreload)await self.registration.navigationPreload.enable().catch(()=>{});await self.clients.claim();await broadcast({type:'TEAM_BULLS_SW_ACTIVATED',version:APP_VERSION});})());});
self.addEventListener('message',event=>{const type=event.data?.type;if(type==='SKIP_WAITING'){event.waitUntil(self.skipWaiting());return;}if(type==='GET_VERSION'){event.source?.postMessage?.({type:'TEAM_BULLS_VERSION',version:APP_VERSION});return;}if(type==='REFRESH_APP_SHELL'){event.waitUntil((async()=>{const ok=await prepareShell().then(()=>true).catch(()=>false);event.source?.postMessage?.({type:'TEAM_BULLS_REFRESHED',ok,version:APP_VERSION});})());return;}if(type==='CLEAR_APP_CACHES'){event.waitUntil((async()=>{const keys=await caches.keys();await Promise.all(keys.filter(key=>key.startsWith(CACHE_PREFIX)&&key!==AUDIO_CACHE_NAME).map(key=>caches.delete(key)));await prepareShell().catch(()=>false);event.source?.postMessage?.({type:'TEAM_BULLS_CACHES_CLEARED',version:APP_VERSION});})());return;}if(type!=='CACHE_AUDIO')return;const raw=String(event.data?.url||'');let url;try{url=new URL(raw,self.location.href);}catch(error){return;}const name=url.pathname.split('/').pop()||'';if(url.origin!==self.location.origin||!AUDIO_NAME_PATTERN.test(name))return;const asset=scopedUrl('./'+name);event.waitUntil((async()=>{const cache=await caches.open(AUDIO_CACHE_NAME);if(await cache.match(asset))return;try{const response=await fetch(asset,{cache:'no-cache'});if(response.ok)await cache.put(asset,response.clone());}catch(error){}})());});
...
async function navigationNetworkFirst(request,event){const url=new URL(request.url);const recovery=/\/recuperar\.html$/.test(url.pathname);const fallback=scopedUrl(recovery?'./recuperar.html':'./index.html');try{const preload=await event.preloadResponse.catch(()=>null);const response=preload?.ok?preload:await fetchWithTimeout(request,NETWORK_TIMEOUT_MS,{cache:'no-store'});if(response?.ok)return secureResponse(await cacheHtml(response,fallback),{html:true});}catch(error){}const cached=await (await caches.open(SHELL_CACHE)).match(fallback);if(cached)return secureResponse(cached,{html:true});return secureResponse(new Response('<!doctype html><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>Team Bulls</title><body style="background:#0c0c0c;color:#eee;font-family:system-ui;padding:24px"><h1>Team Bulls</h1><p>Sem conexão e sem uma cópia offline disponível. Conecte-se e tente novamente.</p><button onclick="location.reload()">Tentar novamente</button></body>',{status:503,headers:{'Content-Type':'text/html; charset=utf-8'}}),{html:true});}
...
async function cacheFirst(request,{cacheName=SHELL_CACHE}={}){const cache=await caches.open(cacheName);const cached=await cache.match(request,{ignoreSearch:false});if(cached)return secureResponse(cached);try{const response=await fetch(request);if(response?.ok)await cache.put(request,response.clone());return secureResponse(response);}catch(error){return new Response('',{status:504});}}
async function audioResponse(request,url){const cache=await caches.open(AUDIO_CACHE_NAME);const cached=await cache.match(url.href);if(cached&&!request.headers.has('range'))return secureResponse(cached);try{const response=await fetchWithTimeout(request,AUDIO_NETWORK_TIMEOUT_MS);if(response?.ok&&!request.headers.has('range'))await cache.put(url.href,response.clone());return secureResponse(response);}catch(error){return cached?secureResponse(cached):new Response('',{status:504});}}

self.addEventListener('fetch',event=>{const request=event.request;if(request.method!=='GET')return;const url=new URL(request.url);if(url.origin!==self.location.origin)return;const relativePath=pathWithinScope(url);const fileName=url.pathname.split('/').pop()||'';if(AUDIO_NAME_PATTERN.test(fileName)){event.respondWith(audioResponse(request,url));return;}if(request.mode==='navigate'){event.respondWith(navigationNetworkFirst(request,event));return;}if(relativePath==='/version.json'){event.respondWith(networkFirst(request,{cacheName:SHELL_CACHE,timeout:3000}));return;}if(MUTABLE_PATHS.has(relativePath)){event.respondWith(networkFirst(request,{cacheName:SHELL_CACHE}));return;}if(VERSIONED_PATH_PATTERN.test(fileName)||url.searchParams.has('v')){event.respondWith(cacheFirst(request,{cacheName:SHELL_CACHE}));return;}event.respondWith(networkFirst(request,{cacheName:RUNTIME_CACHE}));});
