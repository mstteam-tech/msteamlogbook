(()=>{
  ...
  async function savePayment(){
    ...
    if(!studentMap.has(studentId)){toast('Escolha um aluno válido.',true);return;}if(!['quarterly','semiannual'].includes(planType)){toast('Escolha o plano trimestral ou semestral.',true);return;}if(!Number.isFinite(amount)||amount<0||amount>1000000){toast('Informe um valor de pagamento válido.',true);return;}if(!iso(validFrom)){toast('Informe a data de início da vigência.',true);return;}
    const nextDueDate=addCalendarMonths(validFrom,planMonths(planType)),id=paymentEditorId||('pay-'+Date.now().toString(36)+'-'+Math.random().toString(36).slice(2,8)),existing=paymentEditorId?paymentRecords.find(item=>item.id===paymentEditorId):null,button=document.getElementById('tb-payment-save');if(button){button.disabled=true;button.textContent='SALVANDO...';}
    let newPath='';try{
    ...
    }catch(error){if(newPath&&!existing?.receiptPath)deleteReceipt(newPath);toast('Não foi possível salvar o pagamento: '+String(error?.message||error),true);}finally{if(button){button.disabled=false;button.textContent='SALVAR PAGAMENTO';}}
  }
  ...
  function installTrainerHooks(){
    if(typeof renderTrainer==='function'&&!renderTrainer.__tbInboxPayments){const base=renderTrainer;const wrapped=async function(){const result=await base.apply(this,arguments);if(trainer())initTrainer().catch(error=>console.warn('Central do treinador',error));ensureUi();updateBadges();return result;};wrapped.__tbInboxPayments=true;renderTrainer=wrapped;}
  ...
  function boot(){installSubmissionHooks();installTrainerHooks();ensureUi();if(trainer())initTrainer().catch(error=>console.warn('Central do treinador',error));}
