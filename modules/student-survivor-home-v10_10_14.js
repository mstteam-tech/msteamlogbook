/* Team Bulls v10.10.14 — home do aluno preservando a identidade survivor horror. */
'use strict';
(()=>{
  if(window.__TEAM_BULLS_STUDENT_SURVIVOR_HOME_1__)return;
  window.__TEAM_BULLS_STUDENT_SURVIVOR_HOME_1__=true;

  const VERSION='10.10.14-home1';
  ...
  function ensureStyles(){
    if(document.getElementById('tb-survivor-home-style'))return;
    const style=document.createElement('style');
    style.id='tb-survivor-home-style';
    style.textContent=`
      ...
      .tb-feedback-compact.is-open .tb-feedback-chevron{color:#d2c2b6}
  ...
  function ensureHotbar(){
    ...
    shell=document.createElement('div');shell.id='tb-hotbar-shell';shell.className='tb-hotbar-shell';shell.setAttribute('aria-label','Navegação principal do aluno');
  ...
  function setHotbarActive(key){
    document.querySelectorAll('.tb-student-hotbar button').forEach(button=>button.classList.toggle('is-active',button.dataset.hotbar===key));
  }
  ...
  function syncHotbar(){
    ensureHotbar();
    const user=student(),id=activeScreen(),visible=!!user&&!['screen-loading','screen-auth'].includes(id)&&!id.includes('trainer')&&!id.startsWith('screen-ts-');
  ...
  function reduceStudentNavigation(){
    ...
    if(desktop){
      const blocked=['openTechniques','openExerciseOptions','openPhotos',"openV107Operations('notices')",'openCalendar'];
  ...
  function ensureHomeInsights(){
    ...
    let host=document.getElementById('tb-home-intelligence');
    if(host)return host;
  ...
  function feedbackTypeLabel(data){
    const type=String(data?.feedbackType||'general');
    if(type==='protocol_update')return'ATUALIZAÇÃO COMPLETA';
    if(type==='weekly_report')return'RELATÓRIO SEMANAL';
    return'TRANSMISSÃO';
  }
  ...
  async function fetchStudentFeedbacks(uid){
    ...
    try{
      const query=db.collection('feedback').where('studentId','==',uid).limit(100);
  ...
  function renderFeedbacks(items){
    const host=document.getElementById('tb-home-feedbacks');if(!host)return;
    ...
    items.forEach((item,index)=>{
      ...
      const meta=document.createElement('small');const date=fmtDate(item.createdAt);meta.textContent=`${feedbackTypeLabel(item)}${date!=='—'?' // '+date:''}${item.read===true?'':' // NOVO'}`;
  ...
  async function fetchWeightHistory(uid){
    if(!cloudReady())return[];
    try{
      const query=db.collection('progressPhotos').where('userId','==',uid).limit(200);
  ...
  function renderWeightChart(points){
    ...
    if(coords.length>1){const poly=svgNode('polyline',{points:coords.map(p=>`${p.x.toFixed(1)},${p.y.toFixed(1)}`).join(' '),fill:'none',stroke:'#a42b38','stroke-width':2.2});svg.appendChild(poly);}
    coords.forEach((item,index)=>{const circle=svgNode('circle',{cx:item.x,cy:item.y,r:3.2,fill:'#d7c8bc',stroke:'#a42b38','stroke-width':1.5});svg.appendChild(circle);if(index===0||index===coords.length-1||coords.length<=5){const label=svgNode('text',{x:item.x,y:height-9,fill:'#655d57','font-size':6.5,'font-family':'DM Mono, monospace','text-anchor':index===0?'start':index===coords.length-1?'end':'middle'});label.textContent=fmtDate(item.point.date).slice(0,5);svg.appendChild(label);}});
    panel.appendChild(svg);host.appendChild(panel);
  }

  function applyStudentHomeStructure(){
    if(!student())return;
    reduceStudentNavigation();ensureNextDateSlots();ensureHomeInsights();
  }
  ...
  function observeScreens(){
    screenObservers.forEach(observer=>observer.disconnect());screenObservers=[];
    document.querySelectorAll('.screen').forEach(screen=>{const observer=new MutationObserver(records=>{if(records.some(record=>record.attributeName==='class'))syncHotbar();});observer.observe(screen,{attributes:true,attributeFilter:['class']});screenObservers.push(observer);});
  }
  ...
  function boot(){
    ...
    document.addEventListener('team-bulls:student-home-refresh',()=>scheduleRefresh(true));
    setInterval(()=>{const user=student();if(user){syncHotbar();if(activeScreen()==='screen-home')scheduleRefresh(false);}else document.documentElement.classList.remove('tb-student-hotbar-on');},5000);
    window.TeamBullsStudentSurvivorHome=Object.freeze({version:VERSION,refresh:()=>refreshHome(true),syncHotbar});
  }
  ...
  function waitForApp(){
    ...
    if(bootTries<80)setTimeout(waitForApp,100);
  }
