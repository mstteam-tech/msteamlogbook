...
(()=>{
  ...
  function patchInstructionPrompt(){
    ...
    if(typeof maybePromptInitialInstructions==='function'&&!maybePromptInitialInstructions.__tbPromptV2){const wrapped=async function(){if(CURRENT_USER?.role==='trainer'||typeof activeScreenId==='function'&&activeScreenId()!=='screen-home'||instructionAcknowledged())return;const token=instructionIdentity()+'|'+instructionToken();if(!instructionIdentity()||forcedPromptToken===token)return;forcedPromptToken=token;try{await loadGeneralInstructions?.();if(!visibleInstructionCount?.()||instructionAcknowledged())return;await openInstructions(true);}catch(error){console.warn('[Team Bulls] Falha ao abrir orientações iniciais',error);forcedPromptToken='';}};wrapped.__tbPromptV2=true;maybePromptInitialInstructions=wrapped;}
