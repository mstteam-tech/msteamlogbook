/* Team Bulls v10.9.0 — central de operações otimizada, modelos, adesão e notificações. */
'use strict';
(function(){
  ...
  window.v107SelectTab=async function(tab){
    ...
    try{
      ...
      console.error('Central v10.9.0',error);const el=host();
  ...
  async function renderOverview(token){
    ...
    el.innerHTML=`
      <div class="v107-hero"><div><span>TEAM BULLS V10.9.0</span><h2>CONTINUIDADE E CONTROLE</h2><p>${escHtml(studentLabel)} · ${online?'conectado':'offline'}</p></div><div class="v107-status-dot ${online?'ok':'warn'}">${online?'ONLINE':'OFFLINE'}</div></div>
  ...
  async function renderSync(token){
    ...
    el.innerHTML=`<div class="v107-section-head"><div><h2>CENTRAL DE SINCRONIZAÇÃO</h2><p>Mostra o estado real do aparelho e permite tentar novamente sem apagar dados.</p></div><button class="btn-primary" onclick="v107RunSync()">SINCRONIZAR AGORA</button></div>
      ...
      <div class="v107-security-card"><strong>DADOS PRESERVADOS</strong><span>A sincronização não limpa treinos, sessões, fotos ou dietas.</span><small>Para trocar arquivos do GitHub, use a página recuperar.html?v=10.9.0 após a publicação.</small></div>`;
  }
