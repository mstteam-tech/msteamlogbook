(()=>{
  ...
  async function syncCustomFoodsFromCatalog(force=false){
    ...
    customCatalogLoading=(async()=>{
      try{
        ...
        try{window.TeamBullsDietLiveCalories?.refresh?.();}catch(error){console.warn('[Team Bulls] Falha ao recalcular dieta após carregar alimentos personalizados',error);}
        return count;
  ...
  window.addEventListener('pageshow',()=>{applyVersionLabels();installCustomFoodMacroBridge();refreshDietTotalsFromCustomFoods();syncCustomFoodsFromCatalog(true).catch(()=>0);},{passive:true});
})();