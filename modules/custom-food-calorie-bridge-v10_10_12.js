(()=>{
  ...
  function notifyReady(){
    try{window.dispatchEvent(new CustomEvent('team-bulls-custom-food-calories-ready',{detail:{count:cached.length,version:VERSION}}));}catch(error){}
  }
  function apply(items=cached,{notify=false}={}){
  ...
  async function load(force=false){
    ...
    if(!force&&cachedTrainer===uid&&cached.length)return apply(cached,{notify:true});
    ...
    loading=(async()=>{
      try{
        ...
        cached=normalize(snap.exists?snap.data()?.[FIELD]:[]);cachedTrainer=uid;return apply(cached,{notify:true});
      }catch(error){console.warn('[Team Bulls] Não foi possível carregar alimentos personalizados para calorias',error);return apply(cached,{notify:true});}
  ...
  function syncNow(){
    ...
    if(table){cached=table;return apply(cached);}
    if(cached.length)return apply(cached);
  ...
  function schedule({reload=false}={}){
    ...
    refreshFrame=requestAnimationFrame(()=>{
      refreshFrame=0;
      const table=tableItems();
      if(table){cached=table;apply(cached,{notify:true});return;}
      if(reload)load(true).catch(()=>0);else if(cached.length)apply(cached,{notify:true});else load().catch(()=>0);
    });
  }
  ...
  function beforeMealInput(event){
    ...
    // Capturing phase: presets personalizados entram ANTES dos listeners do workspace calcularem kcal/macros.
    syncNow();
  }
  function install(){
  ...
  window.addEventListener('team-bulls-custom-food-catalog-changed',()=>load(true).catch(()=>0));
