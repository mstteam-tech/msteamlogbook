...
(()=>{
  let requested=false,deferredStarted=false,deferredBatchCount=0,deferredCursor=0,studentPriorityStarted=false,healing=false,healTimer=null,readyResolved=false,hadFailures=false,screenObserver=null;
  ...
  const loadDeferred=async()=>{
    ...
    try{
      ...
      while(deferredCursor<modules.length&&sessionUiReady()){
        preloadAhead(deferredCursor);
        const src=modules[deferredCursor++];await loadScript(src);
        await yieldUi();
      }
    }finally{deferredStarted=false;}
    if(deferredCursor>=modules.length){if(failedModules.size){if(typeof showToast==='function')showToast('Conexão instável: alguns recursos continuam sendo finalizados automaticamente.',true);scheduleHeal(1200);}else markReady();}
  };
