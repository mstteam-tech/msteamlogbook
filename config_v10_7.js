...
(()=>{
  ...
  const healFailedModules=async()=>{if(healing||!failedModules.size||navigator.onLine===false)return false;healing=true;const pending=[...failedModules];preloadModules(pending.slice(0,6));try{for(const src of pending)await loadScript(src,9000);}finally{healing=false;}if(failedModules.size)scheduleHeal(5000);else markReady();return failedModules.size===0;};
  const loadDeferred=async()=>{if(deferredStarted)return;deferredStarted=true;document.documentElement.dataset.teamBullsRuntime='loading';for(const src of modules)await loadScript(src);if(failedModules.size){if(typeof showToast==='function')showToast('Conexão instável: alguns recursos continuam sendo finalizados automaticamente.',true);scheduleHeal(1200);}else markReady();};
  const scheduleDeferred=()=>{const queue=()=>{preloadAhead(0);if('requestIdleCallback'in window)requestIdleCallback(()=>loadDeferred(),{timeout:900});else setTimeout(()=>loadDeferred(),160);};const afterPaint=()=>requestAnimationFrame(queue);if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',afterPaint,{once:true});else afterPaint();};
  const load=async()=>{if(requested)return;requested=true;for(const src of criticalModules)await loadScript(src,6500);scheduleDeferred();};
  preloadModules(criticalModules);
  window.TeamBullsRuntimeLoader=Object.freeze({version:'10.10.13-startup5',ready,state:runtimeDetail,retry:healFailedModules});
  window.addEventListener('online',()=>scheduleHeal(500));document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible')scheduleHeal(700);});window.addEventListener('pageshow',()=>scheduleHeal(900));if(window.TeamBulls107)load();else window.addEventListener('team-bulls-v107-ready',load,{once:true});
})();
